...
export const importFilterSchema = z.enum([
  'ALL',
  'ACTIVE',
  'ARRIVING',
  'DELAYED',
...
export const createImportBoxSchema = z
  .object({
    ...
    value.items.forEach((item, index) => {
      ...
      if (seen.has(key)) {
        context.addIssue({
          code: 'custom',
          path: ['items', index],
          message: 'La misma variante y almacén no pueden repetirse dentro de una caja.',
        });
      }
      seen.add(key);
    });
...
export const createPreorderSaleSchema = z
  .object({
