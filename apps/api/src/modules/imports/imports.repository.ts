...
export interface ImportsRepository {
  ...
  createPreorder(input: CreatePreorderSaleInput, idempotencyKey: string): Promise<PreorderSaleResult>;
  ...
  createInsuranceClaim(importId: string, input: CreateInsuranceClaimInput): Promise<ImportGenericResult>;
  updateInsuranceClaim(claimId: string, input: UpdateInsuranceClaimInput): Promise<ImportGenericResult>;
  allocatePreorder(input: AllocatePreorderInput): Promise<ImportGenericResult>;
  receiveBox(boxId: string, input: ReceiveImportBoxInput, idempotencyKey: string): Promise<ImportMutationResult>;
}
