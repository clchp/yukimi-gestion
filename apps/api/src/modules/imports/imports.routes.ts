...
export function createImportsRouter(
  ...
  router.post('/partners', async (request, response, next) => {
    try {
      response.status(201).json({
        data: await serviceFor(request).createPartner(
          createImportPartnerSchema.parse(request.body),
        ),
      });
    } catch (error) {
      next(error);
  ...
  router.post('/preorders', async (request, response, next) => {
    try {
      ...
      response.status(201).json({
        data: await serviceFor(request).createPreorder(
          createPreorderSaleSchema.parse(request.body),
          key,
        ),
      });
    } catch (error) {
  ...
  router.post('/preorders/allocate', async (request, response, next) => {
    try {
      response.status(201).json({
        data: await serviceFor(request).allocatePreorder(
          allocatePreorderSchema.parse(request.body),
        ),
      });
    } catch (error) {
      next(error);
  ...
  router.post('/:importId/state', async (request, response, next) => {
    try {
      const importId = z.string().uuid().parse(request.params.importId);
      response.json({
        data: await serviceFor(request).advance(
          importId,
          updateImportStateSchema.parse(request.body),
        ),
      });
    } catch (error) {
      next(error);
  ...
  router.post('/:importId/costs', async (request, response, next) => {
    try {
      const importId = z.string().uuid().parse(request.params.importId);
      response.status(201).json({
        data: await serviceFor(request).addCost(
          importId,
          createImportCostSchema.parse(request.body),
        ),
      });
    } catch (error) {
      next(error);
  ...
  router.post('/:importId/incidents', async (request, response, next) => {
    try {
      const importId = z.string().uuid().parse(request.params.importId);
      response.status(201).json({
        data: await serviceFor(request).createIncident(
          importId,
          createImportIncidentSchema.parse(request.body),
        ),
      });
    } catch (error) {
      next(error);
  ...
  router.post('/:importId/insurance-claims', async (request, response, next) => {
    try {
      const importId = z.string().uuid().parse(request.params.importId);
      response.status(201).json({
        data: await serviceFor(request).createInsuranceClaim(
          importId,
          createInsuranceClaimSchema.parse(request.body),
        ),
      });
    } catch (error) {
      next(error);
  ...
  router.patch('/insurance-claims/:claimId', async (request, response, next) => {
    try {
      ...
      response.json({
        data: await serviceFor(request).updateInsuranceClaim(
          claimId,
          updateInsuranceClaimSchema.parse(request.body),
        ),
      });
    } catch (error) {
      next(error);
  ...
  router.post('/boxes/:boxId/state', async (request, response, next) => {
    try {
      ...
      response.json({
        data: await serviceFor(request).advanceBox(
          boxId,
          updateImportBoxStateSchema.parse(request.body),
        ),
      });
    } catch (error) {
      next(error);
