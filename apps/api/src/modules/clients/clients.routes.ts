...
function serviceFor(request: Request, clientFactory: UserSupabaseClientFactory) {
  return new ClientService(
    new SupabaseClientRepository(clientFactory.create(accessTokenOrThrow(request.currentAccessToken))),
  );
}
...
export function createClientsRouter(
  ...
  router.post('/', async (request, response, next) => {
    try {
      ...
      response.status(201).json({ data: await serviceFor(request, clientFactory).create(input, idempotencyKey) });
    } catch (error) { next(error); }
  });
  ...
  router.post('/:clientId/addresses', async (request, response, next) => {
    try {
      ...
      response.status(201).json({ data: await serviceFor(request, clientFactory).saveAddress(clientId, null, input, null) });
    } catch (error) { next(error); }
  });
  ...
  router.put('/:clientId/addresses/:addressId', async (request, response, next) => {
    try {
      ...
      response.json({ data: await serviceFor(request, clientFactory).saveAddress(clientId, addressId, input, input.version) });
    } catch (error) { next(error); }
  });
  ...
  router.post('/:clientId/incidents', async (request, response, next) => {
    try {
      ...
      response.status(201).json({ data: await serviceFor(request, clientFactory).createIncident(clientId, input) });
    } catch (error) { next(error); }
  });
  ...
  router.patch('/incidents/:incidentId/resolve', async (request, response, next) => {
    try {
      ...
      response.json({ data: await serviceFor(request, clientFactory).resolveIncident(incidentId, input) });
    } catch (error) { next(error); }
  });
