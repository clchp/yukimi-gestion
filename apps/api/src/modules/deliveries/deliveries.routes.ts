...
export function createDeliveriesRouter(
  ...
  function serviceFor(request: Request) {
    return new DeliveriesService(
      new SupabaseDeliveriesRepository(
  ...
  router.get('/support-data', async (request, response, next) => {
    try {
      const saleId = request.query.saleId
        ? z.string().uuid().parse(request.query.saleId)
        : undefined;
  ...
  router.get('/:deliveryId', async (request, response, next) => {
    try {
      response.json({
        data: await serviceFor(request).getById(z.string().uuid().parse(request.params.deliveryId)),
      });
    } catch (error) {
      next(error);
    }
