...
export function createPaymentsRouter(
  ...
  function serviceFor(request: Request) {
    return new PaymentsService(
      new SupabasePaymentsRepository(
  ...
  router.get('/sales/:saleId', async (request, response, next) => {
    try {
      response.json({
        data: await serviceFor(request).getSaleFinancials(
          z.string().uuid().parse(request.params.saleId),
        ),
      });
    } catch (error) {
      next(error);
  ...
  router.post('/sales/:saleId', async (request, response, next) => {
    try {
      ...
      response
        .status(201)
        .json({ data: await serviceFor(request).createPayment(saleId, input, key) });
    } catch (error) {
      next(error);
    }
  });
  ...
  router.post('/:paymentId/attachments', async (request, response, next) => {
    try {
      ...
      response
        .status(201)
        .json({ data: await serviceFor(request).registerPaymentAttachment(paymentId, input) });
    } catch (error) {
      next(error);
    }
  });
  ...
  router.post('/sales/:saleId/receipts', async (request, response, next) => {
    try {
      ...
      response
        .status(201)
        .json({ data: await serviceFor(request).createReceipt(saleId, input, key) });
    } catch (error) {
      next(error);
    }
  });
  ...
  router.post('/receipts/:receiptId/attachments', async (request, response, next) => {
    try {
      ...
      response
        .status(201)
        .json({ data: await serviceFor(request).registerReceiptAttachment(receiptId, input) });
    } catch (error) {
      next(error);
    }
  });
  ...
  router.post('/receipts/:receiptId/credit-notes', async (request, response, next) => {
    try {
      ...
      response
        .status(201)
        .json({ data: await serviceFor(request).createCreditNote(receiptId, input) });
    } catch (error) {
      next(error);
    }
  });
  ...
  router.post('/sales/:saleId/late-penalty', async (request, response, next) => {
    try {
      response.json({
        data: await serviceFor(request).calculateLatePenalty(
          z.string().uuid().parse(request.params.saleId),
        ),
      });
    } catch (error) {
