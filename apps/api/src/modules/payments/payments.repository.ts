...
export interface PaymentsRepository {
  ...
  createPayment(saleId: string, input: CreatePaymentInput, idempotencyKey: string): Promise<PaymentMutationResult>;
  registerPaymentAttachment(paymentId: string, input: PaymentAttachmentRegistrationInput): Promise<{ id: string }>;
  ...
  reversePayment(paymentId: string, reason: string, idempotencyKey: string): Promise<GenericMutationResult>;
  createReceipt(saleId: string, input: CreateReceiptInput, idempotencyKey: string): Promise<GenericMutationResult>;
  registerReceiptAttachment(receiptId: string, input: ReceiptAttachmentRegistrationInput): Promise<{ id: string }>;
