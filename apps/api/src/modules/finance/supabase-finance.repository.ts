...
export class SupabaseFinanceRepository implements FinanceRepository {
  ...
  public async listTransactions(query: {
  ...
  public async createCategory(input: CreateFinanceCategoryInput) {
    const { data, error } = await this.client.rpc('create_financial_category_v1', {
      p_input: input,
    });
  ...
  public async updateCategory(categoryId: string, input: UpdateFinanceCategoryInput) {
    const { data, error } = await this.client.rpc('update_finance_category_v1', {
      p_category_id: categoryId,
      p_input: input,
    });
    if (error) throw mapSupabaseError(error, 'No se pudo actualizar la categoría.');
    return data as {
      id: string;
      code: string;
      name: string;
  ...
  public async createManualTransaction(
    ...
    const { data, error } = await this.client.rpc('create_manual_financial_transaction_v1', {
      p_input: input,
  ...
  public async createTransfer(
    ...
    const { data, error } = await this.client.rpc('create_financial_transfer_v1', {
      p_input: input,
      p_idempotency_key: idempotencyKey,
    });
  ...
  public async reverseTransaction(
    ...
    const { data, error } = await this.client.rpc('reverse_financial_transaction_v1', {
  ...
  public async registerAttachment(
    ...
    const { data, error } = await this.client.rpc('register_financial_attachment_v1', {
      p_transaction_id: transactionId,
      p_input: input,
    });
    if (error)
      throw mapSupabaseError(error, 'El archivo se subió, pero no se pudo asociar al movimiento.');
  ...
  public async createObligation(
    ...
    const { data, error } = await this.client.rpc('create_obligation_v2', {
      p_input: input,
      p_idempotency_key: idempotencyKey,
    });
  ...
  public async payObligation(
    obligationId: string,
    input: PayObligationInput,
    idempotencyKey: string,
  ): Promise<FinanceMutationResult> {
    const { data, error } = await this.client.rpc('pay_obligation_v1', {
  ...
  public async createReceivedLoan(
    input: CreateReceivedLoanInput,
    idempotencyKey: string,
  ): Promise<FinanceMutationResult> {
    const { data, error } = await this.client.rpc('create_received_loan_v1', {
      p_input: input,
      p_idempotency_key: idempotencyKey,
    });
  ...
  public async payLoanInstallment(
    installmentId: string,
    input: PayLoanInstallmentInput,
    idempotencyKey: string,
  ): Promise<FinanceMutationResult> {
    const { data, error } = await this.client.rpc('pay_loan_installment_v1', {
  ...
  public async createCashClosure(
    input: CreateCashClosureInput,
    idempotencyKey: string,
  ): Promise<CashClosureResult> {
    const { data, error } = await this.client.rpc('create_cash_closure_v1', {
      p_input: input,
      p_idempotency_key: idempotencyKey,
    });
  ...
  public async importBankStatement(
  ...
  public async getBankReconciliation(
    accountId?: string | undefined,
    batchId?: string | undefined,
  ): Promise<BankReconciliationData> {
    const { data, error } = await this.client.rpc('get_bank_reconciliation_v1', {
  ...
  public async confirmBankReconciliation(
    rowId: string,
  ...
  public async dismissBankCandidate(
    candidateId: string,
    reason: string,
  ): Promise<FinanceMutationResult> {
    const { data, error } = await this.client.rpc('dismiss_bank_candidate_v1', {
  ...
  public async ignoreBankRow(rowId: string, reason: string): Promise<FinanceMutationResult> {
    const { data, error } = await this.client.rpc('ignore_bank_row_v1', {
      p_row_id: rowId,
      p_reason: reason,
    });
  ...
  public async reverseBankReconciliation(
    reconciliationId: string,
    reason: string,
  ): Promise<FinanceMutationResult> {
    const { data, error } = await this.client.rpc('reverse_bank_reconciliation_v1', {
