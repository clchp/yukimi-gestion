...
export function createFinanceRouter(authGateway: SupabaseAuthGateway, clientFactory: UserSupabaseClientFactory): Router {
  const router = Router();
  router.use(requireAuth(authGateway));

  function serviceFor(request: Request): FinanceService {
    return new FinanceService(new SupabaseFinanceRepository(clientFactory.create(accessTokenOrThrow(request.currentAccessToken))));
  }
  ...
  router.get('/dashboard', async (request, response, next) => {
    try { response.json({ data: await serviceFor(request).getDashboard() }); } catch (error) { next(error); }
  });
  ...
  router.post('/categories', async (request, response, next) => {
    try { response.status(201).json({ data: await serviceFor(request).createCategory(createFinanceCategorySchema.parse(request.body)) }); } catch (error) { next(error); }
  });

  router.post('/transactions', async (request, response, next) => {
    try {
      const key = request.header('idempotency-key')?.trim() || randomUUID();
      response.status(201).json({ data: await serviceFor(request).createManualTransaction(createManualFinanceTransactionSchema.parse(request.body), key) });
    } catch (error) { next(error); }
  });
  ...
  router.post('/transfers', async (request, response, next) => {
    try {
      const key = request.header('idempotency-key')?.trim() || randomUUID();
      response.status(201).json({ data: await serviceFor(request).createTransfer(createFinanceTransferSchema.parse(request.body), key) });
    } catch (error) { next(error); }
  });
  ...
  router.post('/transactions/:transactionId/reverse', async (request, response, next) => {
    try {
      ...
      const key = request.header('idempotency-key')?.trim() || randomUUID();
      response.json({ data: await serviceFor(request).reverseTransaction(transactionId, reason, key) });
    } catch (error) { next(error); }
  });
  ...
  router.post('/transactions/:transactionId/attachments', async (request, response, next) => {
    try {
      const transactionId = z.string().uuid().parse(request.params.transactionId);
      response.status(201).json({ data: await serviceFor(request).registerAttachment(transactionId, financeAttachmentRegistrationSchema.parse(request.body)) });
    } catch (error) { next(error); }
  });
  ...
  router.post('/obligations', async (request, response, next) => {
    try {
      const key = request.header('idempotency-key')?.trim() || randomUUID();
      response.status(201).json({ data: await serviceFor(request).createObligation(createObligationSchema.parse(request.body), key) });
    } catch (error) { next(error); }
  });
  ...
  router.post('/obligations/:obligationId/pay', async (request, response, next) => {
    try {
      ...
      const key = request.header('idempotency-key')?.trim() || randomUUID();
      response.json({ data: await serviceFor(request).payObligation(obligationId, payObligationSchema.parse(request.body), key) });
    } catch (error) { next(error); }
  });
  ...
  router.post('/loans', async (request, response, next) => {
    try {
      const key = request.header('idempotency-key')?.trim() || randomUUID();
      response.status(201).json({ data: await serviceFor(request).createReceivedLoan(createReceivedLoanSchema.parse(request.body), key) });
    } catch (error) { next(error); }
  });
  ...
  router.post('/loan-installments/:installmentId/pay', async (request, response, next) => {
    try {
      ...
      const key = request.header('idempotency-key')?.trim() || randomUUID();
      response.json({ data: await serviceFor(request).payLoanInstallment(installmentId, payLoanInstallmentSchema.parse(request.body), key) });
    } catch (error) { next(error); }
  });
  ...
  router.post('/cash-closures', async (request, response, next) => {
    try {
      const key = request.header('idempotency-key')?.trim() || randomUUID();
      response.status(201).json({ data: await serviceFor(request).createCashClosure(createCashClosureSchema.parse(request.body), key) });
    } catch (error) { next(error); }
  });
  ...
  router.get('/bank-reconciliation', async (request, response, next) => {
    try {
      ...
      response.json({ data: await serviceFor(request).getBankReconciliation(query.accountId, query.batchId) });
    } catch (error) { next(error); }
  });
  ...
  router.post('/bank-statements/import', async (request, response, next) => {
    try {
      const key = request.header('idempotency-key')?.trim() || randomUUID();
      response.status(201).json({ data: await serviceFor(request).importBankStatement(importBankStatementSchema.parse(request.body), key) });
    } catch (error) { next(error); }
  });
  ...
  router.post('/bank-rows/:rowId/reconcile', async (request, response, next) => {
    try {
      ...
      response.json({ data: await serviceFor(request).confirmBankReconciliation(rowId, confirmBankReconciliationSchema.parse(request.body)) });
    } catch (error) { next(error); }
  });
  ...
  router.post('/bank-reconciliations/:reconciliationId/reverse', async (request, response, next) => {
    try {
      const reconciliationId = z.string().uuid().parse(request.params.reconciliationId);
      const { reason } = financeActionReasonSchema.parse(request.body);
      response.json({ data: await serviceFor(request).reverseBankReconciliation(reconciliationId, reason) });
    } catch (error) { next(error); }
  });
