...
export function createInsightsRouter(authGateway: SupabaseAuthGateway, clientFactory: UserSupabaseClientFactory): Router {
  const router = Router();
  router.use(requireAuth(authGateway));

  function serviceFor(request: Request): InsightsService {
    return new InsightsService(new SupabaseInsightsRepository(clientFactory.create(accessTokenOrThrow(request.currentAccessToken))));
  }
  ...
  router.get('/notifications', async (request, response, next) => {
    try {
      ...
      response.json({ data: await serviceFor(request).getNotifications(query.limit, query.status) });
  ...
  router.patch('/notifications/:notificationId/status', async (request, response, next) => {
    try {
      ...
      response.json({ data: await serviceFor(request).setNotificationStatus(notificationId, input.status) });
    } catch (error) { next(error); }
  });
  ...
  router.get('/reports', async (request, response, next) => {
    try {
      ...
      response.json({ data: await serviceFor(request).getReports(query.startDate, query.endDate, query.warehouseId) });
    } catch (error) { next(error); }
  });

  router.post('/reports/exports', async (request, response, next) => {
    try { response.status(201).json({ data: await serviceFor(request).registerReportExport(registerReportExportSchema.parse(request.body)) }); } catch (error) { next(error); }
  });
