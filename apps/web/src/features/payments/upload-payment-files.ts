...
export async function uploadPaymentProof(paymentId: string, file: File): Promise<void> {
  ...
  const { error } = await supabase.storage.from('payment-proofs').upload(path, file, { cacheControl: '3600', contentType: file.type, upsert: false });
  if (error) throw error;
  try {
    await registerPaymentAttachment(paymentId, {
      bucketId: 'payment-proofs', objectPath: path, originalFilename: file.name,
      mimeType: file.type as 'image/jpeg' | 'image/png' | 'image/webp' | 'application/pdf', sizeBytes: file.size,
    });
...
export async function uploadReceiptFile(receiptId: string, file: File): Promise<void> {
  ...
  const { error } = await supabase.storage.from('receipt-files').upload(path, file, { cacheControl: '3600', contentType: file.type, upsert: false });
  if (error) throw error;
  try {
    await registerReceiptAttachment(receiptId, {
      bucketId: 'receipt-files', objectPath: path, originalFilename: file.name,
      mimeType: file.type as 'image/jpeg' | 'image/png' | 'image/webp' | 'application/pdf', sizeBytes: file.size,
    });
  } catch (registrationError) {
