...
const insuranceStatuses: UpdateInsuranceClaimInput['status'][] = ['PENDING', 'SUBMITTED', 'APPROVED', 'PARTIALLY_APPROVED', 'REJECTED', 'PAID', 'CLOSED'];
...
type InsuranceDraft =
  | { mode: 'CREATE'; incidentId: string }
  | { mode: 'UPDATE'; claimId: string };
...
export function ImportDetailPage() {
  ...
  const [incidentTarget, setIncidentTarget] = useState<{ boxId: string | null; itemId: string | null } | null>(null);
  ...
  const shipmentTransition = useMutation({
    mutationFn: async ({ nextStateCode, reason, trackingNumber }: { nextStateCode: ImportStateCode; reason: string; trackingNumber: string }) => {
  ...
  const boxTransition = useMutation({
    mutationFn: async ({ boxId, nextStateCode, reason, trackingNumber }: { boxId: string; nextStateCode: ImportBoxStateCode; reason: string; trackingNumber: string }) => {
  ...
  const incident = useMutation({
    mutationFn: async () => {
      ...
      return createImportIncident(importId as string, {
        ...
        incidentType,
        affectedQuantity: incidentTarget.itemId && incidentQuantity ? Number(incidentQuantity) : null,
  ...
  const insuranceClaim = useMutation({
    mutationFn: async () => {
      if (!insuranceDraft || insuranceDraft.mode !== 'CREATE') throw new Error('No se seleccionó una incidencia.');
  ...
  const insuranceUpdate = useMutation({
    mutationFn: async () => {
      if (!insuranceDraft || insuranceDraft.mode !== 'UPDATE') throw new Error('No se seleccionó un reclamo.');
  ...
  const error = shipmentTransition.error ?? boxTransition.error ?? receive.error ?? addCost.error ?? incident.error ?? insuranceClaim.error ?? insuranceUpdate.error ?? createPreorder.error ?? preorder.error;
        ...
        <div className="import-detail-main">
          ...
          <Panel title="Cajas" subtitle="Cada caja mantiene su propio tracking, productos y estado.">
            <div className="import-box-detail-list">{data.boxes.map((box) => <article className="import-box-detail" key={box.id}>
              ...
              <div className="responsive-table-wrap"><table className="data-table"><thead><tr><th>Producto</th><th>Destino</th><th>Esperado</th><th>Recibido</th><th>Preventa</th><th>Costo</th><th /></tr></thead><tbody>{box.items.map((item) => <tr key={item.id}><td><strong>{item.productName}</strong><small>{item.variantName} · {item.sku}</small></td><td>{item.destinationWarehouseName ?? 'Sin almacén'}</td><td className="numeric-cell">{item.expectedQuantity}</td><td className="numeric-cell">{item.receivedQuantity}</td><td className="numeric-cell">{item.preorderAllocatedQuantity}</td><td className="numeric-cell">{item.finalUnitCostPen == null ? `${item.originalCurrencyCode} ${item.originalUnitCost}` : money(item.finalUnitCostPen)}</td><td><button className="icon-button" title="Registrar incidencia" onClick={() => { setIncidentTarget({ boxId: box.id, itemId: item.id }); setIncidentType('DAMAGED'); setIncidentDescription(''); setIncidentQuantity('1'); }}><AlertTriangle size={16} /></button></td></tr>)}</tbody></table></div>
              <div className="import-box-actions">
                {box.allowedTransitions.map((transition) => <button key={transition.stateCode} className={transition.stateCode === 'CANCELLED' ? 'button button-danger' : 'button button-secondary'} disabled={boxTransition.isPending} onClick={() => { setTransitionDraft({ scope: 'BOX', boxId: box.id, nextStateCode: transition.stateCode, trackingNumber: box.trackingNumber ?? '' }); setTransitionReason(transitionLabels[transition.stateCode] ?? transition.name); }}><Truck size={16} /> {transitionLabels[transition.stateCode] ?? transition.name}</button>)}
                  ...
                  <div className="incident-content">
                    ...
                    {item.insuranceClaims.map((claim) => (
                      <div className="insurance-claim-row" key={claim.id}>
                        ...
                        {!['REJECTED', 'CLOSED'].includes(claim.status) ? <button className="button button-secondary button-compact" disabled={insuranceUpdate.isPending} onClick={() => { setInsuranceDraft({ mode: 'UPDATE', claimId: claim.id }); setClaimStatus(claim.status as UpdateInsuranceClaimInput['status']); setApprovedAmount(claim.approvedAmount == null ? '' : String(claim.approvedAmount)); setClaimNotes(''); }}>Actualizar</button> : null}
                      </div>
                    ))}
                    {['MISSING', 'DAMAGED'].includes(item.incidentType) && item.insuranceClaims.length === 0 ? <button className="button button-secondary button-compact" disabled={insuranceClaim.isPending} onClick={() => { setInsuranceDraft({ mode: 'CREATE', incidentId: item.id }); setClaimNumber(''); setClaimAmount('0'); setClaimCurrency('PEN'); setClaimNotes(''); }}>Registrar reclamo al seguro</button> : null}
                  </div>
                </article>
              ))}
            </div>
            <button className="button button-secondary button-full" onClick={() => { setIncidentTarget({ boxId: null, itemId: null }); setIncidentType('DELAY'); setIncidentDescription(''); setIncidentQuantity('1'); }}><AlertTriangle size={16} /> Registrar incidencia general</button>
          </Panel>
        </aside>
      </section>

      {transitionDraft ? (
        <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setTransitionDraft(null); }}>
          <form className="modal-card" role="dialog" aria-modal="true" aria-labelledby="import-transition-title" onSubmit={(event) => {
            event.preventDefault();
            if (transitionDraft.scope === 'IMPORT') {
              shipmentTransition.mutate({ nextStateCode: transitionDraft.nextStateCode, reason: transitionReason, trackingNumber: transitionDraft.trackingNumber });
            } else {
              boxTransition.mutate({ boxId: transitionDraft.boxId, nextStateCode: transitionDraft.nextStateCode, reason: transitionReason, trackingNumber: transitionDraft.trackingNumber });
            }
          }}>
            <div className="modal-header">
              <div><small>{transitionDraft.scope === 'IMPORT' ? 'Importación general' : 'Caja de importación'}</small><h2 id="import-transition-title">{transitionLabels[transitionDraft.nextStateCode] ?? stateLabels[transitionDraft.nextStateCode]}</h2><p>El cambio quedará registrado en el historial.</p></div>
              <button className="icon-button" type="button" aria-label="Cerrar" onClick={() => setTransitionDraft(null)}><X size={18} /></button>
            </div>
            {['SHIPPED', 'IN_TRANSIT'].includes(transitionDraft.nextStateCode) ? <label className="field"><span>Tracking {transitionDraft.scope === 'IMPORT' ? 'maestro' : 'de caja'}</span><input value={transitionDraft.trackingNumber} onChange={(event) => setTransitionDraft((current) => current ? { ...current, trackingNumber: event.target.value } : current)} placeholder="Opcional" /></label> : null}
            <label className="field"><span>{transitionDraft.nextStateCode === 'CANCELLED' ? 'Motivo de cancelación' : 'Detalle o referencia'} *</span><textarea rows={4} minLength={3} maxLength={1000} value={transitionReason} onChange={(event) => setTransitionReason(event.target.value)} required /></label>
            <div className="modal-actions"><button className="button button-secondary" type="button" onClick={() => setTransitionDraft(null)}>Volver</button><button className={transitionDraft.nextStateCode === 'CANCELLED' ? 'button button-danger' : 'button button-primary'} type="submit" disabled={shipmentTransition.isPending || boxTransition.isPending}>{shipmentTransition.isPending || boxTransition.isPending ? 'Guardando…' : 'Confirmar cambio'}</button></div>
          </form>
        </div>
      ) : null}

      {incidentTarget ? (
        <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setIncidentTarget(null); }}>
          <form className="modal-card" role="dialog" aria-modal="true" aria-labelledby="import-incident-title" onSubmit={(event) => { event.preventDefault(); incident.mutate(); }}>
            <div className="modal-header">
              <div><small>Trazabilidad logística</small><h2 id="import-incident-title">Registrar incidencia</h2><p>{incidentTarget.itemId ? 'La incidencia quedará vinculada al producto seleccionado.' : 'Incidencia general de la importación.'}</p></div>
              <button className="icon-button" type="button" aria-label="Cerrar" onClick={() => setIncidentTarget(null)}><X size={18} /></button>
            </div>
            <div className="form-grid form-grid-2">
              <label className="field"><span>Tipo *</span><select value={incidentType} onChange={(event) => setIncidentType(event.target.value as ImportIncidentType)}>{Object.entries(incidentLabels).map(([code, label]) => <option value={code} key={code}>{label}</option>)}</select></label>
              {incidentTarget.itemId ? <label className="field"><span>Cantidad afectada</span><input type="number" min="1" step="1" value={incidentQuantity} onChange={(event) => setIncidentQuantity(event.target.value)} /></label> : null}
            </div>
            <label className="field"><span>Descripción *</span><textarea rows={4} minLength={3} maxLength={1000} value={incidentDescription} onChange={(event) => setIncidentDescription(event.target.value)} required /></label>
            <div className="modal-actions"><button className="button button-secondary" type="button" onClick={() => setIncidentTarget(null)}>Volver</button><button className="button button-primary" type="submit" disabled={incident.isPending}>{incident.isPending ? 'Registrando…' : 'Guardar incidencia'}</button></div>
          </form>
        </div>
      ) : null}

      {insuranceDraft ? (
        <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setInsuranceDraft(null); }}>
          <form className="modal-card" role="dialog" aria-modal="true" aria-labelledby="insurance-claim-title" onSubmit={(event) => { event.preventDefault(); if (insuranceDraft.mode === 'CREATE') insuranceClaim.mutate(); else insuranceUpdate.mutate(); }}>
            <div className="modal-header">
              <div><small>Seguro de importación</small><h2 id="insurance-claim-title">{insuranceDraft.mode === 'CREATE' ? 'Registrar reclamo' : 'Actualizar reclamo'}</h2><p>Conserva la referencia, los importes y el detalle de cada decisión.</p></div>
              <button className="icon-button" type="button" aria-label="Cerrar" onClick={() => setInsuranceDraft(null)}><X size={18} /></button>
            </div>
            {insuranceDraft.mode === 'CREATE' ? <div className="form-grid form-grid-2"><label className="field"><span>Número o referencia</span><input value={claimNumber} onChange={(event) => setClaimNumber(event.target.value)} /></label><label className="field"><span>Importe reclamado *</span><input type="number" min="0" step="0.01" value={claimAmount} onChange={(event) => setClaimAmount(event.target.value)} required /></label><label className="field"><span>Moneda *</span><select value={claimCurrency} onChange={(event) => setClaimCurrency(event.target.value)}>{support.data?.currencies.map((currency) => <option value={currency.code} key={currency.code}>{currency.code}</option>)}</select></label></div> : <div className="form-grid form-grid-2"><label className="field"><span>Estado *</span><select value={claimStatus} onChange={(event) => setClaimStatus(event.target.value as UpdateInsuranceClaimInput['status'])}>{insuranceStatuses.map((status) => <option value={status} key={status}>{status}</option>)}</select></label>{['APPROVED', 'PARTIALLY_APPROVED', 'PAID', 'CLOSED'].includes(claimStatus) ? <label className="field"><span>Importe aprobado</span><input type="number" min="0" step="0.01" value={approvedAmount} onChange={(event) => setApprovedAmount(event.target.value)} /></label> : null}</div>}
            <label className="field"><span>{insuranceDraft.mode === 'CREATE' ? 'Detalle del reclamo' : 'Detalle de la actualización'} {insuranceDraft.mode === 'UPDATE' ? '*' : ''}</span><textarea rows={4} minLength={insuranceDraft.mode === 'UPDATE' ? 3 : undefined} maxLength={1000} value={claimNotes} onChange={(event) => setClaimNotes(event.target.value)} required={insuranceDraft.mode === 'UPDATE'} /></label>
            <div className="modal-actions"><button className="button button-secondary" type="button" onClick={() => setInsuranceDraft(null)}>Volver</button><button className="button button-primary" type="submit" disabled={insuranceClaim.isPending || insuranceUpdate.isPending}>{insuranceClaim.isPending || insuranceUpdate.isPending ? 'Guardando…' : 'Confirmar'}</button></div>
          </form>
        </div>
      ) : null}
