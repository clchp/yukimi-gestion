...
export function ClientsPage() {
  ...
  const totalPages = Math.max(
    1,
    Math.ceil((clients.data?.total ?? 0) / (clients.data?.pageSize ?? 20)),
  );
    ...
    <main className="page">
      ...
      <section className="summary-strip">
        <div>
          ...
          <strong>{clients.data?.summary.activeClients ?? '—'}</strong>
        </div>
        <div>
          <span>Clientes VIP</span>
          <strong>{clients.data?.summary.vipClients ?? '—'}</strong>
        </div>
        <div>
          <span>Saldo pendiente</span>
          <strong>{clients.data ? formatMoney(clients.data.summary.pendingBalance) : '—'}</strong>
        </div>
        <div>
          <span>Con pagos vencidos</span>
          <strong className="text-danger">{clients.data?.summary.overdueClients ?? '—'}</strong>
        </div>
      </section>
      ...
      <section className="panel table-panel">
        ...
        <div className="filter-chips">
          {filters.map((item) => (
            ...
            >
              ...
              {item.code === 'OVERDUE' ? (
                <span>{clients.data?.summary.overdueClients ?? 0}</span>
              ) : null}
            </button>
          ))}
        </div>
                ...
                <tr>
                  <td colSpan={7}>
                    <div className="empty-state">
                      ...
                      <p>Crea el primer cliente o cambia los filtros.</p>
                    </div>
                  </td>
                </tr>
              ) : null}
                ...
                <tr key={client.id} onClick={() => navigate(`/clientes/${client.id}`)}>
                  <td>
                    <div className="client-cell">
                      ...
                      <div>
                        ...
                        <small>
                          {client.code} · {client.phone ?? 'Sin celular'}
                        </small>
                      </div>
                    </div>
                  </td>
                  <td>
                    {client.isVip ? (
                      ...
                      <StatusBadge>{client.isActive ? 'Regular' : 'Inactivo'}</StatusBadge>
                    )}
                  </td>
                  <td className="numeric-cell">
                    <strong>{formatMoney(client.totalPurchased)}</strong>
                  </td>
                  <td className="numeric-cell">
                    <strong className={client.balanceAmount > 0 ? 'text-warning' : ''}>
                      {formatMoney(client.balanceAmount)}
                    </strong>
                  </td>
                  <td>
                    {client.overdueSales > 0 ? (
                      <StatusBadge tone="danger">{client.overdueSales} vencida(s)</StatusBadge>
                    ) : (
        ...
        <div className="table-footer">
          <span>
            {clients.data
              ? `Mostrando ${clients.data.items.length} de ${clients.data.total} clientes`
              : 'Cargando…'}
          </span>
        ...
        {clients.data?.items.map((client) => (
          ...
          >
            <div className="mobile-record-header">
              ...
              {client.isVip ? <StatusBadge tone="primary">VIP</StatusBadge> : null}
            </div>
            <div className="mobile-record-grid">
              <span>
                Total comprado<strong>{formatMoney(client.totalPurchased)}</strong>
              </span>
              <span>
                Saldo<strong>{formatMoney(client.balanceAmount)}</strong>
              </span>
              <span>
                Última compra<strong>{formatDate(client.lastPurchaseAt)}</strong>
              </span>
              <span>
                Estado
                <strong>
                  {client.overdueSales ? `${client.overdueSales} vencida(s)` : 'Al día'}
                </strong>
              </span>
            </div>
          </article>
