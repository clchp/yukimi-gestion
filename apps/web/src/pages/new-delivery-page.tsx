...
export function NewDeliveryPage() {
  ...
  const generalSupport = useQuery({ queryKey: ['delivery-support'], queryFn: () => getDeliverySupportData() });
  ...
  useEffect(() => {
    if (selectedSale?.addresses.length && !selectedSale.addresses.some((item) => item.id === addressId)) {
      setAddressId(selectedSale.addresses.find((item) => item.isPrimary)?.id ?? selectedSale.addresses[0]?.id ?? '');
    }
    if (selectedSale) {
  ...
  const save = useMutation({
    mutationFn: async () => {
      ...
      if (selectedItems.length === 0) throw new Error('Selecciona al menos un producto para la entrega.');
      for (const item of selectedItems) {
        const quantity = quantities[item.saleItemId] ?? 0;
        if (quantity > item.remainingQuantity) throw new Error(`La cantidad de ${item.productName} supera lo pendiente.`);
      }
    ...
    onError: (error) => setLocalError(error instanceof Error ? error.message : 'No se pudo crear la entrega.'),
  });
  ...
  return (
    <main className="page delivery-form-page">
      ...
      <PageHeader eyebrow="Nueva operación logística" title="Preparar entrega" description="Selecciona una venta, define los productos de esta salida y registra el método de entrega." />
      {localError ? <div className="alert alert-error">{localError}</div> : null}
      {generalSupport.isError || selectedSupport.isError ? <div className="alert alert-error">No se pudieron cargar las opciones de entrega.</div> : null}

      <section className="delivery-form-layout">
        <div className="delivery-form-main">
          <Panel title="1. Venta y cliente" subtitle="Solo aparecen ventas con productos pendientes de entrega.">
            <label className="field"><span>Venta</span><select value={saleId} onChange={(event) => { const value = event.target.value; setSaleId(value); setSearchParams(value ? { saleId: value } : {}); setQuantities({}); }}><option value="">Selecciona una venta</option>{generalSupport.data?.eligibleSales.map((sale) => <option key={sale.id} value={sale.id}>{sale.code} · {sale.clientName} · {sale.remainingUnits} unidad(es)</option>)}</select></label>
            {selectedSale ? <div className="selection-row"><span className="selection-icon"><Truck size={20} /></span><div><strong>{selectedSale.clientName}</strong><small>{selectedSale.code} · {selectedSale.clientPhone ?? 'Sin celular'}</small></div><StatusBadge tone="info">{selectedSale.deliveryStateCode}</StatusBadge></div> : null}
          </Panel>

          <Panel title="2. Productos de esta entrega" subtitle="Puedes preparar una entrega parcial y dejar el resto pendiente.">
            {!selectedSale ? <div className="empty-state">Selecciona una venta para ver sus productos.</div> : selectedSale.items.length === 0 ? <div className="empty-state"><strong>Sin productos pendientes</strong><p>Todo el contenido de esta venta ya fue asignado a entregas.</p></div> : <div className="delivery-item-picker">{selectedSale.items.map((item) => <article key={item.saleItemId}><div><strong>{item.productName}</strong><small>{item.variantName} · {item.sku}</small>{item.allocations.map((allocation, index) => <small key={`${allocation.warehouseName}-${index}`}>{allocation.warehouseName}: {allocation.quantity} · {allocation.status}</small>)}</div><div className="delivery-quantity-control"><span>Pendiente {item.remainingQuantity}</span><input type="number" min="0" max={item.remainingQuantity} value={quantities[item.saleItemId] ?? 0} onChange={(event) => setQuantities((current) => ({ ...current, [item.saleItemId]: Math.max(0, Number(event.target.value) || 0) }))} /></div></article>)}</div>}
          </Panel>
          ...
          <Panel title="3. Método y destino">
            ...
            <div className="form-grid form-grid-2">
              ...
              <label className="field"><span>N.º de seguimiento inicial</span><input value={trackingNumber} onChange={(event) => setTrackingNumber(event.target.value)} placeholder="Puede registrarse después" /></label>
        ...
        <aside className="delivery-form-summary">
          <Panel title="Resumen"><div className="summary-list"><div><span>Venta</span><strong>{selectedSale?.code ?? '—'}</strong></div><div><span>Cliente</span><strong>{selectedSale?.clientName ?? '—'}</strong></div><div><span>Método</span><strong>{methodOptions.find((item) => item.code === method)?.label}</strong></div><div><span>Unidades</span><strong>{totalUnits}</strong></div><div><span>Costo</span><strong>{money(Number(shippingCost || 0))}</strong></div></div><button className="button button-primary button-full" disabled={save.isPending || !selectedSale || totalUnits === 0} onClick={() => save.mutate()}>{save.isPending ? 'Creando…' : 'Crear entrega'}</button></Panel>
        </aside>
      </section>
    </main>
  );
