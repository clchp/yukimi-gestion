    ...
    <main className="page">
      <PageHeader
        ...
        actions={
          <button
            ...
            onClick={() => navigate('/importaciones/nueva')}
          >
            <Plus size={17} /> Nueva importación
          </button>
        }
      />
      <section className="stat-grid">
        <article className="inventory-stat">
          ...
          <div>
            <small>Importaciones activas</small>
            <strong>{imports.data?.summary.activeImports ?? '—'}</strong>
            <p>Compras y embarques abiertos</p>
          </div>
        </article>
        <article className="inventory-stat">
          ...
          <div>
            <small>Cajas en tránsito</small>
            <strong>{imports.data?.summary.boxesInTransit ?? '—'}</strong>
            <p>Embarcadas o viajando</p>
          </div>
        </article>
        <article className="inventory-stat">
          ...
          <div>
            <small>Unidades esperadas</small>
            <strong>{imports.data?.summary.expectedUnits ?? '—'}</strong>
        ...
        <article className="inventory-stat">
          ...
          <div>
            <small>Retrasadas</small>
            <strong>{imports.data?.summary.delayedImports ?? '—'}</strong>
      ...
      <section className="panel table-panel">
        <Toolbar
          placeholder="Buscar importación, caja, tracking, proveedor o producto…"
          value={search}
          onChange={(value) => {
            setSearch(value);
            setPage(1);
        ...
        <div className="table-footer">
          <span>
            {imports.data
              ? `Mostrando ${imports.data.items.length} de ${imports.data.total} importaciones`
              : 'Cargando…'}
          </span>
