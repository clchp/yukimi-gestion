...
const money = (value: number, currency = 'PEN') => new Intl.NumberFormat('es-PE', { style: 'currency', currency }).format(value);
const date = (value: string) => new Intl.DateTimeFormat('es-PE', { dateStyle: 'medium' }).format(new Date(value));
const dateTimeLocal = () => {
  const now = new Date();
...
type ModalKind = 'movement' | 'transfer' | 'obligation' | 'loan' | 'cash' | 'pay-obligation' | 'pay-loan' | 'reverse' | null;
...
export function FinancePage() {
  ...
  const categoryMutation = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      setNotice(result.reused ? 'La categoría ya existía y fue seleccionada.' : 'Categoría creada.');
      setCategoryDraftOpen(false);
      setNewCategoryName('');
    },
    onError: (value) => setError(value instanceof Error ? value.message : 'No se pudo crear la categoría.'),
  });
  ...
  const movementMutation = useMutation({
    ...
    onSuccess: async (result) => { await refresh(); setNotice(`Movimiento ${result.code ?? ''} registrado.`); closeModal(); },
    onError: (value) => setError(value instanceof Error ? value.message : 'No se pudo registrar el movimiento.'),
  });
  ...
  const transferMutation = useMutation({
    ...
    onSuccess: async (result) => { await refresh(); setNotice(`Transferencia ${result.code ?? ''} registrada.`); closeModal(); },
    onError: (value) => setError(value instanceof Error ? value.message : 'No se pudo registrar la transferencia.'),
  });

  const obligationMutation = useMutation({
    mutationFn: () => createObligation({
      obligationType: obligationType as 'CREDIT_CARD' | 'SUNAT' | 'CUSTOMS' | 'SERVICE' | 'OTHER',
      title: description,
  ...
  const payObligationMutation = useMutation({
    mutationFn: () => {
      ...
      return payObligation(selectedObligation.id, {
        accountId,
        categoryId: null,
        amount: Number(amount),
        occurredAt: new Date(occurredAt).toISOString(),
    ...
    onSuccess: async () => { await refresh(); setNotice('Obligación pagada y gasto registrado.'); closeModal(); },
    onError: (value) => setError(value instanceof Error ? value.message : 'No se pudo pagar la obligación.'),
  });

  const loanMutation = useMutation({
    mutationFn: () => createReceivedLoan({
      lenderName,
    ...
    onSuccess: async () => { await refresh(); setNotice('Préstamo registrado y saldo actualizado.'); closeModal(); },
    onError: (value) => setError(value instanceof Error ? value.message : 'No se pudo registrar el préstamo.'),
  });

  const payLoanMutation = useMutation({
    mutationFn: () => {
      if (!selectedLoan?.nextInstallmentId) throw new Error('No existe una cuota pendiente.');
      return payLoanInstallment(selectedLoan.nextInstallmentId, {
        accountId,
        amount: Number(amount),
        occurredAt: new Date(occurredAt).toISOString(),
        reference: reference.trim() || null,
    ...
    onSuccess: async () => { await refresh(); setNotice('Cuota del préstamo pagada.'); closeModal(); },
    onError: (value) => setError(value instanceof Error ? value.message : 'No se pudo pagar la cuota.'),
  });

  const cashMutation = useMutation({
    mutationFn: () => createCashClosure({
      accountId,
      closureDate,
    ...
    onError: (value) => setError(value instanceof Error ? value.message : 'No se pudo cerrar la caja.'),
  });
  ...
  const reverseMutation = useMutation({
    ...
    onSuccess: async () => { await refresh(); setNotice('Movimiento revertido mediante una operación compensatoria.'); closeModal(); },
    onError: (value) => setError(value instanceof Error ? value.message : 'No se pudo revertir el movimiento.'),
  });

  const accounts = support.data?.accounts ?? [];
  const categories = useMemo(() => (support.data?.categories ?? []).filter((item) => item.nature === movementType || item.nature === 'BOTH'), [support.data, movementType]);
  const cashAccounts = accounts.filter((item) => item.accountTypeCode === 'CASH');
  const totalPen = (dashboard.data?.accounts ?? []).filter((item) => item.currencyCode === 'PEN').reduce((sum, item) => sum + item.currentBalance, 0);
  const maxChart = Math.max(1, ...(dashboard.data?.monthlySummary.flatMap((item) => [item.income, item.expense]) ?? [1]));
  const totalPages = Math.max(1, Math.ceil((transactions.data?.total ?? 0) / (transactions.data?.pageSize ?? 20)));

  function prepareMovement(kind: 'INCOME' | 'EXPENSE') {
    setMovementType(kind);
    setAccountId(accounts[0]?.id ?? '');
    const available = (support.data?.categories ?? []).filter((item) => item.nature === kind || item.nature === 'BOTH');
  ...
  return (
    <main className="page">
      <PageHeader
        ...
        actions={<>
          <button className="button button-secondary" onClick={() => { setSourceAccountId(accounts[0]?.id ?? ''); setDestinationAccountId(accounts[1]?.id ?? ''); setAmount(''); setDescription(''); setOccurredAt(dateTimeLocal()); setModal('transfer'); }}><ArrowRightLeft size={17} /> Transferir</button>
          <button className="button button-primary" onClick={() => prepareMovement('EXPENSE')}><Plus size={17} /> Registrar movimiento</button>
        </>}
      />
      {notice ? <div className="alert alert-success"><CheckCircle2 size={17} /> {notice}</div> : null}
      {dashboard.isError || support.isError ? <div className="alert alert-error">{dashboard.error instanceof Error ? dashboard.error.message : support.error instanceof Error ? support.error.message : 'No se pudo cargar el módulo financiero.'}</div> : null}

      <section className="account-cards">
        <article className="account-balance-card account-featured"><div><span>Saldo total en soles</span><strong>{money(totalPen)}</strong><small>Actualizado con movimientos confirmados</small></div><WalletCards size={30} /></article>
        {(dashboard.data?.accounts ?? []).map((item) => <article className="account-balance-card" key={item.id}><span>{item.name}</span><strong>{money(item.currentBalance, item.currencyCode)}</strong><small>{item.accountTypeCode === 'CASH' ? 'Caja física' : item.currencyCode}</small></article>)}
      </section>
      ...
      <Panel title="Préstamos activos" subtitle="Saldo de capital y próxima cuota">
        <div className="loan-grid">
          {dashboard.data?.loans.length === 0 ? <div className="empty-state">No hay préstamos activos.</div> : null}
          {dashboard.data?.loans.map((loan) => <article className="loan-card" key={loan.id}><div><small>{loan.code}</small><strong>{loan.lenderName}</strong><span>Capital pendiente: {money(loan.outstandingPrincipal, loan.currencyCode)}</span></div><div><small>Próxima cuota</small><strong>{loan.nextInstallmentAmount === null ? 'Sin cuotas' : money(loan.nextInstallmentAmount, loan.currencyCode)}</strong><span>{loan.nextDueDate ? date(`${loan.nextDueDate}T12:00:00`) : '—'}</span></div>{loan.nextInstallmentId ? <button className="button button-secondary button-compact" onClick={() => { setSelectedLoan(loan); setAccountId(accounts[0]?.id ?? ''); setAmount(String(loan.nextInstallmentAmount ?? 0)); setOccurredAt(dateTimeLocal()); setModal('pay-loan'); }}>Pagar cuota</button> : null}</article>)}
        </div>
      </Panel>
      ...
      <Panel className="table-panel mobile-scroll-panel" title="Movimientos financieros" subtitle="Los pagos de ventas aparecen automáticamente cuando se confirman.">
        ...
        <div className="responsive-table-wrap"><table className="data-table"><thead><tr><th>Fecha</th><th>Movimiento</th><th>Categoría</th><th>Cuenta</th><th>Importe</th><th>Estado</th><th>Registrado por</th><th /></tr></thead><tbody>
          {transactions.isLoading ? <tr><td colSpan={8}><div className="empty-state">Cargando movimientos…</div></td></tr> : null}
          {transactions.data?.items.map((item) => { const tone = toneForTransaction(item.transactionTypeCode); const incoming = ['INCOME','LOAN_RECEIVED','LOAN_COLLECTION'].includes(item.transactionTypeCode); return <tr key={item.id}><td>{date(item.occurredAt)}</td><td><div className="movement-cell"><span className={`movement-icon ${incoming ? 'income' : tone === 'info' ? 'transfer' : 'expense'}`}>{incoming ? <ArrowDownLeft size={16} /> : item.transactionTypeCode === 'TRANSFER' ? <ArrowRightLeft size={16} /> : <ArrowUpRight size={16} />}</span><div><strong>{item.description}</strong><small>{item.code}</small></div></div></td><td>{item.categoryName ?? '—'}</td><td>{item.accountNames}</td><td className={`numeric-cell ${incoming ? 'text-success' : item.transactionTypeCode === 'TRANSFER' ? '' : 'text-danger'}`}><strong>{incoming ? '+' : item.transactionTypeCode === 'TRANSFER' ? '' : '−'} {money(item.totalAmount, item.currencyCode)}</strong></td><td><StatusBadge tone={item.stateCode === 'REVERSED' ? 'warning' : tone}>{item.stateCode === 'POSTED' ? 'Confirmado' : item.stateCode}</StatusBadge></td><td>{item.createdByName ?? 'Sistema'}</td><td>{!item.isSystemGenerated && item.sourceType === null && item.stateCode === 'POSTED' && item.transactionTypeCode !== 'REVERSAL' ? <button className="icon-button" title="Revertir" onClick={() => { setSelectedTransactionId(item.id); setReason(''); setModal('reverse'); }}><RotateCcw size={16} /></button> : null}</td></tr>; })}
        </tbody></table></div>
        <div className="pagination"><button disabled={page <= 1} onClick={() => setPage((value) => value - 1)}>Anterior</button><span>Página {page} de {totalPages}</span><button disabled={page >= totalPages} onClick={() => setPage((value) => value + 1)}>Siguiente</button></div>
      </Panel>

      {modal ? <div className="modal-backdrop"><form className="modal-card modal-card-wide" onSubmit={(event) => { event.preventDefault(); setError(null); if (modal === 'movement') movementMutation.mutate(); if (modal === 'transfer') transferMutation.mutate(); if (modal === 'obligation') obligationMutation.mutate(); if (modal === 'pay-obligation') payObligationMutation.mutate(); if (modal === 'loan') loanMutation.mutate(); if (modal === 'pay-loan') payLoanMutation.mutate(); if (modal === 'cash') cashMutation.mutate(); if (modal === 'reverse') reverseMutation.mutate(); }}>
        <div className="modal-header"><div><small>Finanzas</small><h2>{modal === 'movement' ? `Registrar ${movementType === 'INCOME' ? 'ingreso' : 'gasto'}` : modal === 'transfer' ? 'Transferencia entre cuentas' : modal === 'obligation' ? 'Nueva obligación' : modal === 'pay-obligation' ? `Pagar ${selectedObligation?.title ?? 'obligación'}` : modal === 'loan' ? 'Registrar préstamo recibido' : modal === 'pay-loan' ? `Pagar cuota ${selectedLoan?.code ?? ''}` : modal === 'cash' ? 'Cierre de caja' : 'Revertir movimiento'}</h2></div><button type="button" className="icon-button" onClick={closeModal}><X /></button></div>
        ...
        {modal === 'cash' ? <div className="form-grid form-grid-2"><AccountField accounts={cashAccounts} value={accountId} onChange={setAccountId} /><label className="field"><span>Fecha de cierre</span><input type="date" value={closureDate} onChange={(event) => setClosureDate(event.target.value)} /></label><label className="field"><span>Importe contado</span><input type="number" min="0" step="0.01" value={countedAmount} onChange={(event) => setCountedAmount(event.target.value)} /></label><label className="field field-span-2"><span>Notas</span><textarea rows={3} value={notes} onChange={(event) => setNotes(event.target.value)} /></label><label className="field field-span-2"><span>Motivo de diferencia</span><textarea rows={3} value={reason} onChange={(event) => setReason(event.target.value)} placeholder="Obligatorio solo si el contado es diferente al saldo esperado." /></label></div> : null}
        <div className="modal-actions"><button type="button" className="button button-secondary" onClick={closeModal}>Cancelar</button><button className="button button-primary" disabled={movementMutation.isPending || transferMutation.isPending || obligationMutation.isPending || payObligationMutation.isPending || loanMutation.isPending || payLoanMutation.isPending || cashMutation.isPending || reverseMutation.isPending}>Confirmar</button></div>
      </form></div> : null}
    </main>
  );
}

function AccountField({ accounts, value, onChange, label = 'Cuenta' }: { accounts: Array<{ id: string; name: string; currentBalance: number; currencyCode: string }>; value: string; onChange: (value: string) => void; label?: string }) {
  return <label className="field"><span>{label}</span><select value={value} onChange={(event) => onChange(event.target.value)}><option value="">Seleccionar</option>{accounts.map((item) => <option key={item.id} value={item.id}>{item.name} · {money(item.currentBalance, item.currencyCode)}</option>)}</select></label>;
}
function CategoryField({ categories, value, onChange }: { categories: FinanceCategory[]; value: string; onChange: (value: string) => void }) {
  return <label className="field"><span>Categoría</span><select value={value} onChange={(event) => onChange(event.target.value)}><option value="">Seleccionar</option>{categories.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>;
}
function MoneyField({ value, onChange }: { value: string; onChange: (value: string) => void }) {
  return <label className="field"><span>Importe</span><input type="number" min="0.01" step="0.01" value={value} onChange={(event) => onChange(event.target.value)} /></label>;
}
function DateTimeField({ value, onChange, label = 'Fecha y hora' }: { value: string; onChange: (value: string) => void; label?: string }) {
