...
import { Download, Edit3, Eye, ImageIcon, Plus, Printer, QrCode, X } from 'lucide-react';
...
function productStatus(product: ProductItem) {
  ...
  if (stock.available === 0 && stock.preorder > 0)
    return { code: 'PREORDER', label: 'Preventa', tone: 'info' as const };
  if (stock.minimum > 0 && stock.available <= stock.minimum)
    return { code: 'LOW', label: 'Stock bajo', tone: 'warning' as const };
...
export function ProductsPage() {
  ...
  const activeCount =
    Number(Boolean(categoryId)) + Number(Boolean(franchiseId)) + Number(statusFilter !== 'ALL');
  ...
  const qrPayload =
    labelTarget && selectedVariant
      ...
      : '';
  const labelSubtitle = selectedVariant
  ...
  async function loadAllForExport() {
    ...
    return all.filter((product) => {
      if (franchiseId && product.franchiseId !== franchiseId) return false;
      if (
        !['ALL', 'ACTIVE', 'INACTIVE'].includes(statusFilter) &&
        productStatus(product).code !== statusFilter
      )
        return false;
      return true;
    });
  ...
  function openLabel(product: ProductItem) {
    ...
    if (!variant) {
      notify({
        ...
        message: 'No existe una variante para generar la etiqueta.',
        tone: 'warning',
      });
      return;
    }
  ...
  async function downloadLabel() {
    ...
    try {
      ...
      notify({
        ...
        message: `${filename} se guardó en Descargas.`,
        tone: 'success',
      });
    } catch (error) {
  ...
  return (
    <main className="page">
        ...
        actions={
          <>
            <button
              ...
              title="Descarga los resultados filtrados para abrirlos en Excel."
            >
      ...
      {products.isError ? (
        <div className="alert alert-error">No se pudieron cargar los productos.</div>
      ) : null}
      ...
      <section className="panel table-panel">
        ...
        <div className="responsive-table-wrap">
          <table className="data-table product-table">
            ...
            <tbody>
              ...
              {!products.isLoading && visibleItems.length === 0 ? (
                <tr>
                  <td colSpan={9}>
              ...
              {visibleItems.map((product) => {
                ...
                return (
                  <tr key={product.productId}>
                    <td>
                      <div className="product-cell">
                        <button
                          ...
                          type="button"
                          aria-label={`Ver imagen de ${product.productName}`}
                        ...
                        <button
                          ...
                          type="button"
                          onClick={() => navigate(`/productos/${product.productId}`)}
                        >
                    ...
                    <td className="numeric-cell">{stock.reserved}</td>
                    <td className="numeric-cell">{stock.transit}</td>
                    <td className="numeric-cell">
                    ...
                    <td>
                      <div className="table-row-actions">
                        ...
                        <button
                          ...
                          onClick={() => navigate(`/productos/${product.productId}/editar`)}
                        >
                          <Edit3 size={17} />
                        </button>
                        <button
                          ...
                          type="button"
                          title="Etiqueta QR"
                          aria-label={`Etiqueta QR de ${product.productName}`}
        ...
        <div className="table-footer">
          <span>
            Mostrando {visibleItems.length} de {filteredItems.length} productos filtrados
          </span>
        ...
        {visibleItems.map((product) => {
          ...
          return (
            <article className="mobile-record-card" key={product.productId}>
              <button
                className="mobile-record-header clickable-row"
                type="button"
                onClick={() => navigate(`/productos/${product.productId}`)}
              >
                <ProductImage
              ...
              <div className="mobile-record-grid">
                <span>
                  Disponible<strong>{stock.available}</strong>
                </span>
                <span>
                  Reservado<strong>{stock.reserved}</strong>
                </span>
                <span>
                  Tránsito<strong>{stock.transit}</strong>
                </span>
                <span>
      ...
      <FilterPanel
        open={filterOpen}
        activeCount={
          Number(Boolean(draftCategoryId)) +
          Number(Boolean(draftFranchiseId)) +
          Number(draftStatusFilter !== 'ALL')
        }
      ...
      >
        ...
        />
        <ContextNote>
          Los resultados actuales también se aplicarán a la exportación CSV.
        </ContextNote>
      </FilterPanel>
      ...
      {imageTarget ? (
        <div
          ...
          role="presentation"
          onMouseDown={(event) => {
            if (event.target === event.currentTarget) setImageTarget(null);
          }}
        >
          <section
            className="app-modal-card product-image-modal"
            role="dialog"
            aria-modal="true"
          ...
          >
            <header className="app-modal-header">
              <div>
                <span className="eyebrow">Imagen del producto</span>
                <h2 id="product-image-title">{imageTarget.productName}</h2>
                <p>
                  {imageTarget.productCode} · {imageTarget.variants.length}{' '}
                  {imageTarget.variants.length === 1 ? 'variante' : 'variantes'}
                </p>
              </div>
              <button
            ...
            <div className="image-viewer-stage">
              <ProductImage
                path={imageTarget.imagePath}
                alt={imageTarget.productName}
                className="product-detail-image"
                fallbackText={imageTarget.categoryName.slice(0, 1)}
              />
            </div>
            {!imageTarget.imagePath ? (
              <ContextNote tone="warning">
                <ImageIcon size={16} /> Este producto todavía no tiene una imagen asociada.
              </ContextNote>
            ) : null}
            <footer className="app-modal-actions">
              ...
              <button
                className="button button-primary"
                type="button"
                onClick={() => navigate(`/productos/${imageTarget.productId}`)}
              >
                Ver detalle
              </button>
            </footer>
      ...
      {labelTarget && selectedVariant ? (
        ...
        >
          <section
            className="app-modal-card product-label-modal"
            role="dialog"
            aria-modal="true"
          ...
          >
            ...
            {labelTarget.product.variants.length > 1 ? (
              <SearchableSelect
                ...
                options={labelTarget.product.variants.map((variant) => ({
                  value: variant.variantId,
                  label: `${variant.variantName} · ${variant.sku}`,
                }))}
                onChange={(variantId) =>
                  setLabelTarget({ product: labelTarget.product, variantId })
                }
              />
            ) : null}
            <div
