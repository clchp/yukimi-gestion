...
export function NewImportPage() {
  ...
  const filteredVariants = useMemo(() => {
    ...
    return variants
      .filter((item) =>
        `${item.productName} ${item.variantName} ${item.sku} ${item.productCode}`
          .toLowerCase()
          .includes(term),
      )
      .slice(0, 80);
    ...
    boxes.forEach((box) =>
      box.items.forEach((item) => {
        ...
        purchaseValuePen +=
          ...
          Number(item.originalUnitCost || 0) *
          Number(item.exchangeRateToPen || 0);
      }),
    );
  ...
  const createPartner = useMutation({
    mutationFn: async () => {
      ...
      const existing = support.data?.suppliers.find(
        (supplier) => normalizePartnerName(supplier.name) === normalizedName,
      );
    ...
    onError: (error) =>
      setLocalError(error instanceof Error ? error.message : 'No se pudo crear el proveedor.'),
  });
  ...
  const save = useMutation({
    ...
    onError: (error) =>
      setLocalError(error instanceof Error ? error.message : 'No se pudo crear la importación.'),
  });
  ...
  function addBox() {
    setBoxes((current) => [...current, makeBox(purchaseCurrencyCode, sunatExchangeRate)]);
  }
        ...
        box.id === boxId
          ? {
              ...
              items: [
                ...box.items,
                {
                  ...makeItem(purchaseCurrencyCode, sunatExchangeRate),
                  destinationWarehouseId: defaultWarehouse,
                },
              ],
            }
          : box,
  ...
  function validate(): boolean {
    setLocalError(null);
    if (!purchaseCurrencyCode || sunatExchangeRate <= 0) {
      setLocalError('Selecciona la moneda e ingresa un tipo de cambio válido.');
      return false;
    }
    if (boxes.length === 0) {
      setLocalError('Agrega al menos una caja.');
      return false;
    }
    for (const [boxIndex, box] of boxes.entries()) {
      if (box.items.length === 0) {
        setLocalError(`La caja ${boxIndex + 1} no tiene productos.`);
        return false;
      }
      for (const item of box.items) {
        if (!item.variantId || !item.destinationWarehouseId) {
          setLocalError(`Completa producto y almacén en la caja ${boxIndex + 1}.`);
          return false;
        }
        if (
          item.expectedQuantity <= 0 ||
          item.originalUnitCost < 0 ||
          item.exchangeRateToPen <= 0
        ) {
          setLocalError(`Revisa cantidades, costos y tipo de cambio en la caja ${boxIndex + 1}.`);
          return false;
        }
      }
    }
  ...
  return (
    <main className="page import-form-page">
      ...
      <PageHeader
        eyebrow="Compra internacional"
        title="Nueva importación"
        description="Registra la compra, sus cajas y los productos esperados antes de avanzar el flujo logístico."
      />
      ...
      <section className="import-form-layout">
        <div className="import-form-main">
          <Panel title="Datos generales" subtitle="La importación inicia en estado Cotización.">
            <div className="form-grid">
              <label className="field">
                ...
                <div className="field-inline">
                  ...
                  >
                    ...
                    {support.data?.suppliers.map((item) => (
              ...
              <label className="field">
                ...
                <select
                  value={transportMode}
                  onChange={(event) => setTransportMode(event.target.value as ImportTransportMode)}
                >
              ...
              <label className="field">
                <span>Moneda de compra</span>
                <select
                  value={purchaseCurrencyCode}
                  onChange={(event) => {
                    const code = event.target.value;
                    setPurchaseCurrencyCode(code);
                    setBoxes((current) =>
                      current.map((box) => ({
                        ...box,
                        items: box.items.map((item) => ({ ...item, originalCurrencyCode: code })),
                      })),
                    );
                  }}
                >
              ...
              <label className="field">
                ...
                <input
                  ...
                  min="0.000001"
                  step="0.000001"
                  value={sunatExchangeRate}
                  onChange={(event) => setSunatExchangeRate(Number(event.target.value))}
                />
              </label>
              <label className="field">
              ...
              <label className="field field-span-2">
                ...
                <textarea
                  ...
                  placeholder="Orden externa, acuerdos, observaciones…"
                />
              </label>
            </div>
          </Panel>
          ...
          <Panel
            title="Cajas y productos"
            subtitle="Una importación puede contener varias cajas y cada caja varios productos."
          >
            <label className="search-field product-search-large">
              ...
              <input
                ...
                placeholder="Filtrar productos por nombre, SKU o código…"
              />
            </label>
            <div className="import-box-editor-list">
              {boxes.map((box, boxIndex) => (
                <article className="import-box-editor" key={box.id}>
                  <header>
                    <div>
                      ...
                      <div>
                        ...
                        <small>{box.items.length} producto(s)</small>
                      </div>
                    </div>
                    {boxes.length > 1 ? (
                      <button
                  ...
                  <div className="form-grid compact-grid">
                    <label className="field">
                      <span>Operador internacional</span>
                      <select
                        value={box.internationalOperatorId}
                        onChange={(event) =>
                          updateBox(box.id, { internationalOperatorId: event.target.value })
                        }
                      >
                        <option value="">Sin asignar</option>
                        {support.data?.internationalOperators.map((item) => (
                          <option key={item.id} value={item.id}>
                            {item.name}
                          </option>
                        ))}
                    ...
                    <label className="field">
                      <span>Operador local</span>
                      <select
                        value={box.localOperatorId}
                        onChange={(event) =>
                          updateBox(box.id, { localOperatorId: event.target.value })
                        }
                      >
                        <option value="">Sin asignar</option>
                        {support.data?.localOperators.map((item) => (
                          <option key={item.id} value={item.id}>
                            {item.name}
                          </option>
                        ))}
                    ...
                    <label className="field">
                      ...
                      <input
                        ...
                        onChange={(event) =>
                          updateBox(box.id, { trackingNumber: event.target.value })
                        }
                      />
                    </label>
                    <label className="field">
                      ...
                      <input
                        ...
                        onChange={(event) =>
                          updateBox(box.id, { estimatedArrivalDate: event.target.value })
                        }
                      />
                    </label>
                    <label className="field">
                      ...
                      <input
                        ...
                        onChange={(event) =>
                          updateBox(box.id, { weightGrams: Number(event.target.value) })
                        }
                      />
                    </label>
                    <label className="field">
                      ...
                      <input
                        value={box.notes}
                        onChange={(event) => updateBox(box.id, { notes: event.target.value })}
                      />
                    </label>
                  </div>
                  <div className="import-item-editor-list">
                    {box.items.map((item, itemIndex) => (
                      <div className="import-item-editor" key={item.id}>
                        <label className="field import-product-field">
                          <span>Producto {itemIndex + 1}</span>
                          <select
                            value={item.variantId}
                            onChange={(event) =>
                              updateItem(box.id, item.id, { variantId: event.target.value })
                            }
                          >
                        ...
                        <label className="field">
                          ...
                          >
                            ...
                            {support.data?.warehouses.map((warehouse) => (
                        ...
                        <label className="field">
                          ...
                          <input
                            ...
                            onChange={(event) =>
                              updateItem(box.id, item.id, {
                                originalUnitCost: Number(event.target.value),
                              })
                            }
                          />
                        </label>
                        <label className="field">
                          ...
                          <select
                            ...
                            onChange={(event) =>
                              updateItem(box.id, item.id, {
                                originalCurrencyCode: event.target.value,
                              })
                            }
                          >
                            {support.data?.currencies.map((currency) => (
                        ...
                        <label className="field">
                          ...
                          <input
                            ...
                            min="0.000001"
                            step="0.000001"
                            value={item.exchangeRateToPen}
                            onChange={(event) =>
                              updateItem(box.id, item.id, {
                                exchangeRateToPen: Number(event.target.value),
                              })
                            }
                          />
                        </label>
                        <button
                          type="button"
                          className="icon-button danger-icon import-item-remove"
            ...
            <button type="button" className="button button-secondary" onClick={addBox}>
              <Plus size={16} /> Agregar otra caja
            </button>
          </Panel>
        </div>
        ...
        <aside className="import-form-summary">
          <Panel title="Resumen">
            <div className="summary-list">
              ...
              <div>
                <span>Productos</span>
                <strong>{boxes.reduce((sum, box) => sum + box.items.length, 0)}</strong>
              </div>
              <div>
              ...
              <div>
                ...
                <strong>{money(totals.purchaseValuePen)}</strong>
              </div>
              <div>
                <span>Transporte</span>
                <strong>
                  {transportMode === 'AIR' ? 'Avión' : transportMode === 'SEA' ? 'Barco' : 'Otro'}
                </strong>
              </div>
            </div>
            <button
              className="button button-primary button-full"
              disabled={save.isPending || support.isLoading}
              onClick={() => {
                if (validate()) save.mutate();
              }}
            >
              {save.isPending ? 'Creando…' : 'Crear importación'}
            </button>
          </Panel>
        </aside>
      </section>
      {partnerOpen ? (
        <div
          ...
          role="presentation"
          onMouseDown={(event) => {
            if (event.target === event.currentTarget) setPartnerOpen(false);
          }}
        >
          <form
          ...
          >
            <div className="modal-header">
              <div>
                ...
                <h2 id="new-supplier-title">Nuevo proveedor o tienda</h2>
                <p>
                  Si ya existe un nombre equivalente, el sistema seleccionará el registro existente.
                </p>
              </div>
              <button
            ...
            <label className="field">
              <span>País (ISO de 2 letras)</span>
              <input
            ...
            {createPartner.isError ? (
              <div className="alert alert-error">
                {createPartner.error instanceof Error
            ...
            <div className="modal-actions">
              ...
              <button
                ...
                disabled={createPartner.isPending}
              >
                {createPartner.isPending ? 'Creando…' : 'Crear y seleccionar'}
              </button>
            </div>
          </form>
        </div>
