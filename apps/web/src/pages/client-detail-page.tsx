    ...
    <main className="page">
      ...
      <section className="client-profile-grid">
        <Panel className="profile-card">
          ...
          {data.isVip ? <div className="vip-box"><div><span>Condición VIP</span><strong>Activa</strong></div><div><span>Separación</span><strong>Se negocia en cada venta</strong></div><div><span>Plazo especial</span><strong>{data.vipProfile?.paymentTermDays ? `${data.vipProfile.paymentTermDays} días` : 'Por evaluar'}</strong></div></div> : null}
      ...
      {vipOpen ? <div className="modal-backdrop"><form className="modal-card" onSubmit={(event) => { event.preventDefault(); setError(null); vipMutation.mutate(!data.isVip); }}><div className="modal-header"><div><small>Condición especial</small><h2>{data.isVip ? 'Retirar condición VIP' : 'Convertir en cliente VIP'}</h2></div><button className="icon-button" type="button" onClick={() => setVipOpen(false)}><X /></button></div>{!data.isVip ? <div className="form-grid form-grid-2"><div className="alert alert-info field-span-2">El adelanto mínimo no usa un límite fijo: se acordará según los productos de cada venta.</div><label className="field field-span-2 checkbox-field"><input type="checkbox" checked={vipCanReserve} onChange={(event) => setVipCanReserve(event.target.checked)} /><span>Puede negociarse una separación sin adelanto</span></label><label className="field"><span>Plazo especial (días)</span><input type="number" min="1" max="365" value={vipTerm} onChange={(event) => setVipTerm(event.target.value)} /></label></div> : <div className="alert alert-warning"><AlertTriangle size={17} /> El historial VIP se conservará aunque retires el beneficio.</div>}<label className="field"><span>Motivo *</span><textarea rows={4} value={vipReason} onChange={(event) => setVipReason(event.target.value)} placeholder="Frecuencia de compras, incumplimientos u otra evaluación." /></label><div className="modal-actions"><button className="button button-secondary" type="button" onClick={() => setVipOpen(false)}>Cancelar</button><button className="button button-primary" type="submit" disabled={vipMutation.isPending}>{vipMutation.isPending ? 'Guardando…' : 'Confirmar'}</button></div></form></div> : null}
