...
function formatMoney(value: number, currencyCode = 'PEN') {
  return new Intl.NumberFormat('es-PE', { style: 'currency', currency: currencyCode }).format(value);
}
...
export function ClientDetailPage() {
  ...
  const vipMutation = useMutation({
    mutationFn: async (isVip: boolean) => {
      ...
      return setClientVip(clientId, {
        ...
        separationLimitAmount: isVip && vipLimit ? Number(vipLimit) : null,
        separationLimitCurrency: isVip ? 'PEN' : null,
        paymentTermDays: isVip && vipTerm ? Number(vipTerm) : null,
        validUntil: null,
      });
    },
    onSuccess: async () => { setVipOpen(false); setVipReason(''); await refresh(); },
  });

  const addressMutation = useMutation({
    mutationFn: async () => {
      if (!clientId) throw new Error('No se encontró el cliente.');
      if (addressLine.trim().length < 5) throw new Error('La dirección debe tener al menos cinco caracteres.');
      const input = {
    ...
    onSuccess: async () => { closeAddress(); await refresh(); },
  });
  ...
  const incidentMutation = useMutation({
    ...
    onSuccess: async () => { setIncidentOpen(false); setIncidentDescription(''); setIncidentAmount(''); await refresh(); },
  });
  ...
  const statusMutation = useMutation({
    mutationFn: async () => {
      ...
      return setClientStatus(clientId, {
        ...
        reason: client.data.isActive ? 'Cliente desactivado desde su ficha' : 'Cliente reactivado desde su ficha',
      });
    },
  ...
  const primaryAddress = useMemo(
    () => client.data?.addresses.find((address) => address.isDefault && address.isActive) ?? client.data?.addresses.find((address) => address.isActive),
    [client.data],
  );
  ...
  function openVip() {
    ...
    setVipLimit(client.data.vipProfile?.separationLimitAmount?.toString() ?? '');
  ...
  if (client.isError || !client.data) return <main className="page"><div className="alert alert-error">{client.error instanceof Error ? client.error.message : 'No se encontró el cliente.'}</div></main>;

  const data = client.data;

  return (
    <main className="page">
      <button className="back-link" onClick={() => navigate('/clientes')}><ArrowLeft size={17} /> Volver a clientes</button>
      <PageHeader
        title={data.fullName}
        description={`${data.code} · Cliente desde ${formatDate(data.createdAt)}`}
        actions={
          <>
            <button className="button button-secondary" onClick={() => navigate(`/clientes/${data.id}/editar`)}><Edit3 size={17} /> Editar</button>
            <button className="button button-primary" onClick={openVip}><Star size={17} /> Gestionar VIP</button>
          </>
        }
      />

      {visibleError ? <div className="alert alert-error">{visibleError}</div> : null}
      {!data.isActive ? <div className="alert alert-warning">Este cliente está inactivo. Su historial se conserva, pero no debería usarse en ventas nuevas.</div> : null}

      <section className="client-profile-grid">
        <Panel className="profile-card">
          <div className="profile-hero"><span className="profile-avatar">{initials(data.fullName)}</span><div><h2>{data.fullName}</h2>{data.isVip ? <StatusBadge tone="primary"><Star size={12} /> Cliente VIP</StatusBadge> : <StatusBadge>{data.isActive ? 'Cliente regular' : 'Inactivo'}</StatusBadge>}</div></div>
          <div className="contact-list">
            <div><Phone size={17} /><span><small>Celular</small><strong>{data.phone ?? 'No registrado'}</strong></span></div>
            <div><Mail size={17} /><span><small>Correo</small><strong>{data.email ?? 'No registrado'}</strong></span></div>
            <div><MapPin size={17} /><span><small>Dirección principal</small><strong>{primaryAddress?.addressLine ?? 'No registrada'}</strong></span></div>
          </div>
          {data.isVip ? <div className="vip-box"><div><span>Condición VIP</span><strong>Activa</strong></div><div><span>Límite sin adelanto</span><strong>{data.vipProfile?.separationLimitAmount == null ? 'Por evaluar' : formatMoney(data.vipProfile.separationLimitAmount, data.vipProfile.separationLimitCurrency ?? 'PEN')}</strong></div><div><span>Plazo especial</span><strong>{data.vipProfile?.paymentTermDays ? `${data.vipProfile.paymentTermDays} días` : 'Por evaluar'}</strong></div></div> : null}
          <button className={`button ${data.isActive ? 'button-danger-soft' : 'button-secondary'} button-full`} onClick={() => setStatusOpen(true)}>{data.isActive ? <><UserX size={17} /> Desactivar cliente</> : <><CheckCircle2 size={17} /> Reactivar cliente</>}</button>
        </Panel>
        <div className="client-kpis">
          <article><span className="stat-icon stat-primary"><WalletCards size={19} /></span><small>Total comprado</small><strong>{formatMoney(data.stats.totalPurchased)}</strong><p>{data.stats.purchaseCount} compras registradas</p></article>
          <article><span className="stat-icon stat-warning"><CalendarClock size={19} /></span><small>Saldo pendiente</small><strong>{formatMoney(data.stats.balanceAmount)}</strong><p>{data.stats.overdueSales > 0 ? `${data.stats.overdueSales} venta(s) vencida(s)` : 'Sin vencimientos'}</p></article>
          <article><span className="stat-icon stat-info"><ShieldAlert size={19} /></span><small>Incidentes pendientes</small><strong>{data.stats.unresolvedIncidents}</strong><p>Morosidad, liberaciones o devoluciones</p></article>
          <article><span className="stat-icon stat-success"><MapPin size={19} /></span><small>Productos acumulados</small><strong>{data.stats.accumulatedUnits}</strong><p>Unidades pendientes de entrega</p></article>
        </div>
      </section>

      <section className="detail-tabs">
        <button className={tab === 'SUMMARY' ? 'active' : ''} onClick={() => setTab('SUMMARY')}>Resumen</button>
        <button className={tab === 'ADDRESSES' ? 'active' : ''} onClick={() => setTab('ADDRESSES')}>Direcciones</button>
        <button className={tab === 'INCIDENTS' ? 'active' : ''} onClick={() => setTab('INCIDENTS')}>Incumplimientos</button>
        <button className={tab === 'VIP_HISTORY' ? 'active' : ''} onClick={() => setTab('VIP_HISTORY')}>Historial VIP</button>
      </section>

      {tab === 'SUMMARY' ? (
        <section className="dashboard-grid dashboard-grid-primary">
          <Panel title="Ventas recientes" subtitle="Últimas operaciones del cliente">
            <div className="responsive-table-wrap"><table className="data-table"><thead><tr><th>Venta</th><th>Fecha</th><th>Total</th><th>Saldo</th><th>Pago</th><th>Entrega</th></tr></thead><tbody>{data.recentSales.length === 0 ? <tr><td colSpan={6}><div className="empty-state">Todavía no tiene ventas registradas.</div></td></tr> : data.recentSales.map((sale) => <tr key={sale.id} onClick={() => navigate(`/ventas/${sale.id}`)}><td><strong>{sale.code}</strong></td><td>{formatDate(sale.createdAt)}</td><td className="numeric-cell">{formatMoney(sale.totalAmount, sale.currencyCode)}</td><td className="numeric-cell">{formatMoney(sale.balanceAmount, sale.currencyCode)}</td><td><StatusBadge tone={sale.balanceAmount <= 0 ? 'success' : 'warning'}>{sale.paymentStateCode}</StatusBadge></td><td>{sale.deliveryStateCode}</td></tr>)}</tbody></table></div>
          </Panel>
          <Panel title="Notas e información" subtitle="Datos útiles para atender al cliente">
            <div className="note-card"><strong>Notas internas</strong><p>{data.notes ?? 'No se registraron notas.'}</p></div>
            <div className="note-card"><strong>Documento</strong><p>{data.documentNumber ? `${data.documentType ?? ''} ${data.documentNumber}` : 'No registrado'}</p></div>
            {data.incidents.filter((item) => !item.resolvedAt).slice(0, 3).map((incident) => <div className="activity-row" key={incident.id}><span className={`activity-marker ${incident.severity === 'HIGH' ? 'activity-danger' : 'activity-warning'}`} /><div><strong>{incidentLabels[incident.incidentType]}</strong><p>{incident.description}</p></div><time>{formatDate(incident.occurredAt)}</time></div>)}
          </Panel>
        </section>
      ) : null}

      {tab === 'ADDRESSES' ? (
        <Panel title="Direcciones y agencias" subtitle="Mantén varias direcciones y define una como principal." action={<button className="button button-primary button-compact" onClick={() => openAddress()}><Plus size={16} /> Agregar dirección</button>}>
          <div className="address-grid">{data.addresses.length === 0 ? <div className="empty-state">No hay direcciones registradas.</div> : data.addresses.map((address) => <article className={`address-card ${!address.isActive ? 'is-muted' : ''}`} key={address.id}><div className="address-card-head"><div><strong>{address.label}</strong>{address.isDefault ? <StatusBadge tone="primary">Principal</StatusBadge> : null}</div><button className="button button-secondary button-compact" onClick={() => openAddress(address)}>Editar</button></div><p>{address.addressLine}</p><small>{[address.district, address.province, address.department].filter(Boolean).join(', ') || 'Ubicación no especificada'}</small><small>Agencia: {address.preferredPartnerName ?? 'Sin preferencia'}</small>{address.reference ? <small>Referencia: {address.reference}</small> : null}</article>)}</div>
        </Panel>
      ) : null}
      ...
      {tab === 'VIP_HISTORY' ? (
        <Panel title="Historial de condición VIP" subtitle="Cada concesión, actualización o retiro queda registrado.">
          <div className="activity-list simple-activity">{data.vipHistory.length === 0 ? <div className="empty-state">Todavía no tiene movimientos VIP.</div> : data.vipHistory.map((item) => <div className="activity-row" key={item.id}><span className={`activity-marker ${item.action === 'REVOKED' ? 'activity-danger' : 'activity-primary'}`} /><div><strong>{item.action === 'GRANTED' ? 'Condición VIP otorgada' : item.action === 'UPDATED' ? 'Condición VIP actualizada' : 'Condición VIP retirada'}</strong><p>{item.reason} · {item.performedByName ?? 'Sistema'}</p></div><time>{formatDate(item.occurredAt)}</time></div>)}</div>
        </Panel>
      ) : null}

      {vipOpen ? <div className="modal-backdrop"><form className="modal-card" onSubmit={(event) => { event.preventDefault(); setError(null); vipMutation.mutate(!data.isVip); }}><div className="modal-header"><div><small>Condición especial</small><h2>{data.isVip ? 'Retirar condición VIP' : 'Convertir en cliente VIP'}</h2></div><button className="icon-button" type="button" onClick={() => setVipOpen(false)}><X /></button></div>{!data.isVip ? <div className="form-grid form-grid-2"><label className="field field-span-2 checkbox-field"><input type="checkbox" checked={vipCanReserve} onChange={(event) => setVipCanReserve(event.target.checked)} /><span>Puede reservar sin adelanto</span></label><label className="field"><span>Límite de separación (S/)</span><input type="number" min="0" step="0.01" value={vipLimit} onChange={(event) => setVipLimit(event.target.value)} /></label><label className="field"><span>Plazo especial (días)</span><input type="number" min="1" max="365" value={vipTerm} onChange={(event) => setVipTerm(event.target.value)} /></label></div> : <div className="alert alert-warning"><AlertTriangle size={17} /> El historial VIP se conservará aunque retires el beneficio.</div>}<label className="field"><span>Motivo *</span><textarea rows={4} value={vipReason} onChange={(event) => setVipReason(event.target.value)} placeholder="Frecuencia de compras, incumplimientos u otra evaluación." /></label><div className="modal-actions"><button className="button button-secondary" type="button" onClick={() => setVipOpen(false)}>Cancelar</button><button className="button button-primary" type="submit" disabled={vipMutation.isPending}>{vipMutation.isPending ? 'Guardando…' : 'Confirmar'}</button></div></form></div> : null}
