...
const preferenceResultSchema = z.object({ notificationTypeCode: z.string(), version: z.number().int().positive() });
...
export class SupabaseAdminRepository implements AdminRepository {
  ...
  public async getSettings(): Promise<AdminSettings> {
    const [profiles, warehouses, accounts, settings, notificationTypes, preferences] = await Promise.all([
      this.client
        .from('profiles')
        .select('id,display_name,email_snapshot,phone,is_active,version,user_roles!inner(role_code,revoked_at)')
        .eq('user_roles.role_code', 'ADMIN')
        .is('user_roles.revoked_at', null)
        .order('display_name'),
      this.client.from('warehouses').select('id,code,name,warehouse_type,description,is_virtual,is_visible_in_operations,is_active,version').order('name'),
      this.client.from('financial_accounts').select('id,code,name,account_type_code,currency_code,institution_name,masked_account_number,owner_name,linked_parent_account_id,is_active,version').order('name'),
      this.client.from('business_settings').select('setting_key,setting_value,value_type,category,description,is_editable,version').eq('is_editable', true).order('category').order('setting_key'),
      this.client.from('notification_types').select('code,name,description').eq('is_active', true).order('name'),
      this.client.from('notification_preferences').select('notification_type_code,in_app_enabled,push_enabled,email_enabled,quiet_hours_start,quiet_hours_end,version').order('notification_type_code'),
    ]);
    const failure = [profiles, warehouses, accounts, settings, notificationTypes, preferences].find((result) => result.error);
    if (failure?.error) throw mapSupabaseError(failure.error, 'No se pudo cargar la configuración administrativa.');
    ...
    return adminSettingsSchema.parse({
      ...
      notificationTypes: (notificationTypes.data ?? []).map((row) => ({ code: row.code, name: row.name, description: row.description })),
  ...
  public async updateSetting(key: string, input: UpdateBusinessSettingInput) {
    const { data, error } = await this.client.rpc('update_business_setting_v1', { p_key: key, p_value: input.value, p_expected_version: input.version, p_reason: input.reason });
  ...
  public async upsertFinancialAccount(input: UpsertFinancialAccountInput) {
    const { data, error } = await this.client.rpc('upsert_financial_account_v1', { p_input: input });
  ...
  public async updateProfile(profileId: string, input: UpdateAdminProfileInput) {
    const { data, error } = await this.client.rpc('update_admin_profile_v1', { p_profile_id: profileId, p_input: input });
  ...
  public async upsertNotificationPreference(input: NotificationPreferenceInput) {
    const { data, error } = await this.client.rpc('upsert_notification_preference_v1', { p_input: input });
  ...
  public async upsertPushSubscription(input: PushSubscriptionInput) {
    const { data, error } = await this.client.rpc('upsert_push_subscription_v1', { p_input: input });
    if (error) throw mapSupabaseError(error, 'No se pudo registrar este dispositivo para notificaciones push.');
